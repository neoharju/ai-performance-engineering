"""Optimized Layer 4: Concurrency (Ch11-12) - CUDA Graphs.

Incremental optimization showing Layers 1-4 contribution:
- Layers 1-3: TF32, FlashAttention, etc.
- Layer 4: CUDA streams, CUDA graphs

This demonstrates the inter-kernel pipelining techniques from Chapters 11-12.
"""
...
from optimization_layers import Layer01Basics, Layer02Memory, Layer03Pipelining, Layer04Concurrency
...
class OptimizedLayer4Benchmark(BaseBenchmark):
    """Optimized: Layers 1-4 (Ch1-12 basics + pipelining + graphs).
    
    Adds concurrency optimizations:
    - CUDA streams for prefill/decode overlap
    - CUDA graphs for decode loop (reduced launch overhead)
    
    Expected improvement: 1.2-1.5x additional from CUDA graphs
    """
    ...
    def setup(self) -> None:
        """Setup with Layers 1-4 optimizations."""
        ...
        print("[Layer 4] CUDA graphs enabled (for decode phase)")
    ...
    def benchmark_fn(self) -> None:
        with self._nvtx_range("layer4_graphs"):
    ...
    def get_custom_metrics(self) -> Optional[Dict[str, float]]:
        """Return custom metrics for layer 4 CUDA graphs benchmark."""
        if self.last_metrics is None:
            return None
        m = self.last_metrics
        return {
            "layer4_graphs.ttft_ms": m.ttft_ms,
            "layer4_graphs.tpot_ms": m.tpot_ms,
            "layer4_graphs.tokens_per_sec": m.tokens_per_sec,
            "layer4_graphs.peak_memory_gb": m.peak_memory_gb,
        }


def get_benchmark() -> BaseBenchmark:
    """Factory function for harness discovery."""
    return OptimizedLayer4Benchmark()
...
if __name__ == "__main__":
    ...
    print(f"\nLayers 1-4 (Ch1-12): {result.timing.mean_ms:.2f} ms" if result.timing else "No timing")

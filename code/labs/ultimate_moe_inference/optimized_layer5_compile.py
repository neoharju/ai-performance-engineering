"""Optimized Layer 5: PyTorch (Ch13-14) - FP8, torch.compile.

Incremental optimization showing Layers 1-5 contribution:
- Layers 1-4: TF32, FlashAttention, CUDA graphs
- Layer 5: FP8 via Transformer Engine, torch.compile

This demonstrates the PyTorch optimization techniques from Chapters 13-14.
"""
...
class OptimizedLayer5Benchmark(BaseBenchmark):
    """Optimized: Layers 1-5 (Ch1-14 all PyTorch optimizations).
    
    Adds PyTorch optimizations:
    - torch.compile with max-autotune
    - TorchInductor kernel fusion
    
    Expected improvement: 1.3-1.5x additional from torch.compile
    """
    ...
    def setup(self) -> None:
        """Setup with Layers 1-5 optimizations."""
        ...
        # Layer 4: Setup streams
        print("[Layer 4] Setting up CUDA streams...")
        self.layer4.setup_streams()
        
        # Layer 5: Configure inductor and compile
        print("[Layer 5] Configuring TorchInductor...")
        self.layer5.configure_inductor()
        
        print("[Layer 5] Compiling model with torch.compile (max-autotune)...")
        self.model = self.layer5.compile_model(self.model, mode="max-autotune")
        
        # Prepare inputs
        self._prepare_inputs()
        
        # Warmup (required for torch.compile)
        print("Running warmup (includes JIT compilation)...")
        for _ in range(5):
            self._run_inference()
    ...
    def benchmark_fn(self) -> None:
        with self._nvtx_range("layer5_compile"):
    ...
    def get_custom_metrics(self) -> Optional[Dict[str, float]]:
        """Return custom metrics for layer 5 torch.compile benchmark."""
        if self.last_metrics is None:
            return None
        m = self.last_metrics
        return {
            "layer5_compile.ttft_ms": m.ttft_ms,
            "layer5_compile.tpot_ms": m.tpot_ms,
            "layer5_compile.tokens_per_sec": m.tokens_per_sec,
            "layer5_compile.peak_memory_gb": m.peak_memory_gb,
        }


def get_benchmark() -> BaseBenchmark:
    """Factory function for harness discovery."""
    return OptimizedLayer5Benchmark()
...
if __name__ == "__main__":
    ...
    print(f"\nLayers 1-5 (Ch1-14): {result.timing.mean_ms:.2f} ms" if result.timing else "No timing")

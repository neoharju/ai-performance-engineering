"""Optimized runtime scheduling benchmark (async prep + stream interval).

Models vLLM-style async scheduling and stream interval buffering.
"""
...
class OptimizedRuntimeSchedulerBenchmark(VerificationPayloadMixin, BaseBenchmark):
    ...
    def __init__(self) -> None:
        ...
        self.scenarios = (
            SchedulerScenario(
                ...
                decode_steps=16,
                tokens_per_step=16,
                stream_interval=1,  # keep TTFT minimal
                matmul_dim=1024,
            ),
            SchedulerScenario(
                name="throughput",
                ...
                decode_steps=16,
                tokens_per_step=16,
                stream_interval=16,  # buffer tokens to reduce host overhead
                matmul_dim=1536,
            ),
        )
    ...
    def _run_scenario_async(self, scenario: SchedulerScenario) -> float:
...
def get_benchmark() -> BaseBenchmark:
    return OptimizedRuntimeSchedulerBenchmark()
